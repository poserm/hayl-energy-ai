    ...
    <div className="min-h-screen flex relative bg-slate-900">
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 lg:px-8 bg-slate-900/95 backdrop-blur-sm relative z-10">
        <div className="max-w-sm w-full space-y-8">
          ...
          {/* Success Messages */}
          {isVerified && (
            <Alert variant="success" title="Email Verified!" className="animate-fade-in mb-6 bg-green-950 border-green-800 text-green-200">
              Your email has been successfully verified. You can now sign in to your account.
            </Alert>
          )}

          {signupSuccess && (
            <Alert variant="info" title="Account Created!" className="animate-fade-in mb-6 bg-blue-950 border-blue-800 text-blue-200">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            {/* Error Alert */}
            {error && (
              <Alert variant="error" title="Sign in failed" closable onClose={clearError} className="bg-red-950 border-red-800 text-red-200">
                {error}
              </Alert>
            )}
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  required
                  className="block w-full px-3 py-3 pr-10 bg-slate-700 border border-slate-600 text-slate-100 rounded-lg shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200"
                />
                <button
                  type="button"
      ...
      <div className="hidden lg:flex lg:flex-1 lg:items-center lg:justify-center bg-gradient-to-br from-slate-800/80 to-slate-900/80 relative overflow-hidden backdrop-blur-sm">
        ...
        <div className="relative z-10 w-80 h-80 bg-slate-800/95 backdrop-blur-md rounded-2xl shadow-2xl flex items-center justify-center border border-slate-700">
          <div className="text-center">
            ...
            <h3 className="text-xl font-bold text-slate-100 mb-2">Energy Market Intelligence</h3>
            <p className="text-slate-300 text-sm">Real-time data for smarter energy decisions</p>
          </div>
        </div>
      </div>
    </div>
