...
export function AuthProvider({ children, onAuthChange }: AuthProviderProps) {
  ...
  const authApiUrl = process.env.NEXT_PUBLIC_AUTH_API_URL || 'http://localhost:3003'
      ...
      if (data.success && data.user) {
        ...
        if (data.code === 'EMAIL_NOT_VERIFIED') {
          ...
          if (shouldResend && data.data?.email) {
            try {
              const resendResponse = await fetch(`${authApiUrl}/api/auth/verify-email`, {
                method: 'POST',
  ...
  }, [router, searchParams, onAuthChange, authApiUrl])
  ...
  const logout = useCallback(async (redirectTo?: string) => {
    ...
    router.push(targetPath)
  }, [router, onAuthChange, authApiUrl])
